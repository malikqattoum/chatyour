...
if (role(['permissions' => ['coins' => 'coins']])) {
    ...
    if ($noerror) {
        ...
        DB::connect()->update("coin_packages", [
            "name" => $data['name'],
            "coins" => $data['coins'],
            "price" => $data['price'],
            "description" => $data['description'],
            "active" => $isActive,
            "updated_at" => Registry::load('current_user')->time_stamp,
        ...
        if (!DB::connect()->error) {
            ...
            if (isset($_FILES['icon']['name']) && !empty($_FILES['icon']['name'])) {
                if (isImage($_FILES['icon']['tmp_name'])) {
                    ...
                    $filename = 'coin-package-'.$data['package_id'].Registry::load('config')->file_seperator.random_string(['length' => 6]).'.'.$extension;
